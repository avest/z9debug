	...
	public function __invoke()
	{
		...
		switch ($base_name)
		{
			case 'more.php':
				...
				$show_var_data_key = '';
				if (isset($_POST['var_data_key']))
				{
					$show_var_data_key = $_POST['var_data_key'];
				}
				debug::variable($show_var_data_key);

				if (empty($show_var_data_key) && $show_var_data_key !== '0')
				{
					debug::str_exit();
					exit();
				}
				...
				$show_lines_key = '';
				if (isset($_POST['lines_key']))
				{
					$show_lines_key = $_POST['lines_key'];
				}
				debug::variable($show_lines_key);

				if (empty($show_lines_key))
				{
					debug::str_exit();
					exit();
				}
				...
				return $this->display_more($session_id, $request_id, $show_var_data_key, $show_lines_key, $page);
				break;
		}
	}


	public function display_more($session_id, $request_id, $show_var_data_key, $show_lines_key, $page)
	{
		debug::on(false);
		debug::variable($session_id);
		debug::variable($request_id);
		...
		$this->response->setVars(array(
			'var_data' => $var_data,
			'show_var_data_key' => $show_var_data_key,
			'show_lines_key' => $show_lines_key,
			'session_id' => $session_id,
			'request_id' => $request_id,
		));
		$this->response->setView('more.tpl.php', 'Z9\Debug\Console');
